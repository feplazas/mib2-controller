...
export const TELNET_SCRIPTS: TelnetScript[] = [
  ...
  },
  {
    ...
    category: 'backup',
    commands: ['echo "Iniciando backup completo... Esto puede tardar 10-30 minutos." && dd if=/dev/mmcblk0 of=/mnt/sd/backups/mib2_full_$(date +%Y%m%d_%H%M%S).img bs=4M status=progress 2>&1 && echo "Backup completo finalizado."'],
    riskLevel: 'warning',
    requiresConfirmation: true,
  ...
  },
  {
    ...
    category: 'backup',
    commands: ['echo "Iniciando backup de partición 1..." && dd if=/dev/mmcblk0p1 of=/mnt/sd/backups/mib2_p1_$(date +%Y%m%d_%H%M%S).img bs=4M status=progress 2>&1 && echo "Backup de partición 1 finalizado."'],
    riskLevel: 'warning',
    requiresConfirmation: true,
  ...
  },
  {
    ...
    category: 'backup',
    commands: ['echo "Iniciando backup de partición 2..." && dd if=/dev/mmcblk0p2 of=/mnt/sd/backups/mib2_p2_$(date +%Y%m%d_%H%M%S).img bs=4M status=progress 2>&1 && echo "Backup de partición 2 finalizado."'],
    riskLevel: 'warning',
    requiresConfirmation: true,
    warningKey: 'telnet_scripts.dd_backup_partition_warning',
    successKey: 'telnet_scripts.dd_backup_partition_success',
